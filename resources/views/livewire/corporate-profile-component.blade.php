<div class=" bg-gray-300" x-data="{ Overview: @entangle('overview').defer, Operation: @entangle('operationalrea').defer, Financial: @entangle('financial').defer, CorporateNetwork: @entangle('corporatenetwork').defer, Spotlight: @entangle('spotlight').defer }">
    ...
    <div class="py-6 sm:px-6 px-4 " >
        ...
        {{-- Overview --}}
        <div x-show="Overview" x-cloak style="display: none !important" class="h-screen overflow-y-auto prose max-w-none">
            {!! $data->overview !!}
        </div>

         {{-- Operation Area --}}
         <div x-show="Operation" x-cloak style="display: none !important" class="h-screen overflow-y-auto prose max-w-none">
            {!! $data->operationarea !!}
        </div>

        {{-- Financial and ownership --}}
        <div x-show="Financial" x-cloak style="display: none !important" class="h-screen overflow-y-auto prose max-w-none">
            {!! $data->financialownership !!}
        </div>

        {{-- Corporate Network --}}
        <div x-show="CorporateNetwork" x-cloak style="display: none !important" class="h-screen overflow-y-auto  prose max-w-none">
            {!! $data->corporatenetwork !!}
        </div>

        {{-- Spotlight Cases --}}
        <div x-show="Spotlight" x-cloak style="display: none !important" class="h-screen overflow-y-auto prose max-w-none">
            {!! $data->spotlightcases !!}
        </div>
    </div>
